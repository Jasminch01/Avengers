...
const Card = ({ card , selectHandler}) => {
  const {id, name, age, image, salary, role } = card;
  return (
    <div className="p-5 border-2 border-white rounded-xl text-center space-y-5  backdrop-blur-sm">
      ...
      <div className="cart-body">
        ...
        <button onClick={()=>selectHandler(card, id)} className="p-3 backdrop-blur-xl border-white border-2 rounded-lg mt-5">Select</button>
      </div>
    </div>
  );
};
